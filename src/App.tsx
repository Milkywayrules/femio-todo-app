...
const App = () => {
  ...
  return (
    <div className="min-h-screen w-screen bg-gray-l-100 font-main text-gray-l-500">
      {/* bg banner image */}
      ...
      <div className="relative flex flex-col gap-7 px-6 py-11">
        ...
        <main className="flex flex-col gap-5">
          {/* input form */}
          <label
            className="flex w-full items-center justify-between gap-3 overflow-hidden rounded bg-white py-3.5 px-4 text-sm"
            htmlFor="add-todo"
          >
            ...
            <TodoContainer todos={todos} setTodos={setTodos} filterOn={filterOn}>
              {({ id, isComplete, todo }) => {
                ...
                if (isComplete) {
                  ...
                  classNameP = 'line-through text-gray-l-300'
                }

                return (
                  <Fragment key={id}>
                    <div className="flex w-full items-center justify-between gap-3 overflow-hidden bg-white py-3.5 px-4 text-sm">
                      <div
                        className="flex w-full items-center gap-3"
                        onClick={() => handleTodoComplete(id)}
                      >
                        <div
                          className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-full border border-gray-l-300 ${className}`}
                        >
                          {isComplete && (
